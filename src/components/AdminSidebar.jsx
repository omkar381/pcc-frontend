...
const AdminSidebar = ({ onLogout }) => {
  ...
  const menuItems = [
    { path: '/admin/dashboard', name: 'Dashboard', icon: 'fas fa-tachometer-alt' },
    { path: '/admin/students', name: 'Students', icon: 'fas fa-user-graduate' },
    { path: '/admin/add-student', name: 'Add Student', icon: 'fas fa-user-plus' },
    { path: '/admin/attendance', name: 'Attendance', icon: 'fas fa-calendar-check' },
    { path: '/admin/notes', name: 'Notes', icon: 'fas fa-book' },
    { path: '/admin/tests', name: 'Tests', icon: 'fas fa-clipboard-list' },
  ];
  ...
  return (
    <div className={`bg-gradient-to-b from-blue-800 to-blue-900 text-white ${collapsed ? 'w-16' : 'w-64'} min-h-screen flex flex-col transition-all duration-300 shadow-xl z-10`}>
      <div className="p-4 border-b border-blue-700 flex justify-between items-center">
        {!collapsed && <h2 className="text-xl font-bold">PCC Admin</h2>}
        <button 
