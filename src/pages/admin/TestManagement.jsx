...
const TestManagement = ({ onLogout }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch current class
        const classResponse = await axios.get(`${API_URL}/api/admin/current-class`, {
        ...
        if (classResponse.data.selected_class) {
          ...
          const testsResponse = await axios.get(`${API_URL}/api/admin/class-tests`, {
            headers: { Authorization: `Bearer ${token}` }
          });
          
          // Fetch class-specific students
          const studentsResponse = await axios.get(`${API_URL}/api/admin/class-students`, {
            headers: { Authorization: `Bearer ${token}` }
          });
  ...
  const handleAddTest = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(`${API_URL}/api/admin/tests`, newTest, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleSubmitResults = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${API_URL}/api/admin/tests/${selectedTest.id}/results`, {
        results: testResults
      }, {
        headers: { Authorization: `Bearer ${token}` }
      });
